...
import {calculateTotalCredits, isCreditLimitExceeded} from '../utils/creditUtils';
import {courseTypeMap} from '../constants/courseTypes';
import {transformCartData, transformCourseData, transformEnrollmentData} from '../transformers/courseTransformers';
...
const useCourseRegistration = () => {
    ...
    const addToCart = useCallback(async (course) => {
        try {
            ...
            setToast({
                open: true,
                message: err.response?.data?.message || err.message || '장바구니에 추가하는 중 오류가 발생했습니다.',
                severity: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const removeFromCart = useCallback(async (courseIdOrCartId) => {
        try {
            ...
            setToast({
                open: true,
                message: err.response?.data?.message || err.message || '장바구니에서 제거하는 중 오류가 발생했습니다.',
                severity: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const clearAllCarts = useCallback(async () => {
        try {
            ...
            setToast({
                open: true,
                message: err.response?.data?.message || err.message || '장바구니를 비우는 중 오류가 발생했습니다.',
                severity: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const confirmRegistration = useCallback(async () => {
        ...
        try {
            ...
            setToast({
                open: true,
                message: err.response?.data?.message || err.message || '수강신청 중 오류가 발생했습니다.',
                severity: 'error'
            });
        } finally {
            setLoading(false);
    ...
    const handleCancelEnrollment = useCallback(async (enrollmentId) => {
        try {
            ...
            setToast({
                open: true,
                message: err.response?.data?.message || err.message || '수강신청 취소 중 오류가 발생했습니다.',
                severity: 'error'
            });
        } finally {
            setLoading(false);
