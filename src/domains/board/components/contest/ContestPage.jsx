...
import {ArrowBack as ArrowBackIcon, EmojiEvents as EmojiEventsIcon,} from '@mui/icons-material';
...
const ContestPage = () => {
    ...
    // useContest 훅에서 공통 함수 가져오기
    const {fetchContestDetail, handleBackToList} = useContest();
    ...
    useEffect(() => {
        const loadDetail = async () => {
            ...
            setError(null);
            try {
                const data = await fetchContestDetail(id);
                setPost(data);
                ...
                console.error('공모전 게시판 조회 실패:', err);
    ...
    return (
        <Box>
            ...
            <Paper sx={{p: 4}}>
                {/* 헤더 */}
                <PostHeader
                    icon={<EmojiEventsIcon sx={{color: 'primary.main'}}/>}
                    postType={postType}
                    title={post.title}
                ...
                {/* 해시태그 */}
                <PostHashtags hashtags={post.hashtags} boardType="CONTEST"/>
